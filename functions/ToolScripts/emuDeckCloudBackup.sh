...
cloud_backup_install () {
    ...
    if [ ! -f "${rclone_bin}" ]; then
        ...
        mv "${rclone_path}/tmp/"* "${rclone_path}/tmp/rclone" # don't quote the * - I won't
...
cloud_backup_etup () {
    while true; do
        if [ ! -e "${rclone_bin}" ] || [ ! -e "$rclone_jobScript" ];  then
            ans=$(
                zenity --info --title 'cloud_backup' \
...
# createJob
cloud_backup_createJob () {
    echo '#!/usr/bin/env bash'>"${rclone_jobScript}"
    ...
    echo '#!/usr/bin/env bash'>"${rclone_restoreScript}"
    echo "source \$emudeckFolder/settings.sh
    PIDFILE=\"\${toolsPath}/rclone/rclone.pid\"
